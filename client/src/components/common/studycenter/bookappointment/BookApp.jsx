  ...
  export default function BookApp() {
    ...
    const handleChange = (e) => {
      ...
      // Check if the field is 'address' (where spaces are allowed)
      if (name === 'address') {
        // Check if the input contains a link
        const linkRegex = /(http(s)?:\/\/[^\s]+)/gi;
        if (linkRegex.test(value)) {
          // Display an alert error message
          alert('Links are not allowed in the address field');
          return;
        }
        // No links found, proceed with updating the state
        setFormData({ ...formData, [name]: value });
      } else if (name === 'courses') {
        // Limit the number of words for the 'courses' field
        ...
        // Limit the number of words for fullName and fatherName fields
        const words = value.trim().split(/\s+/);
        if (words.length > 20) {
          // Display an alert error message if the limit is exceeded
          alert(`${name} should be less than 20 words`);
          return;
        }
        // No limit exceeded, proceed with updating the state
        setFormData({ ...formData, [name]: value });
      } else {
        // For other fields, remove leading/trailing spaces
...
<div className="mb-4">
                        ...
                        SHAFI acu & ayurvedic, 470, W First Cross St, Mellur, KK Nagar, Madurai, Tamil Nadu 625020
...
Our Vision is to create a drug-less society by 2030 and a Disease-free society by 2030.</p>
                      </div>
                      <div className="mb-4">
                  ...
                  <div className="row gy-3 gx-4">
                    ...
                    <div className="col-xl-6">
                      ...
                      >
                        ...
                        <option value="Diploma In Acupuncture Therapy - D.A.T. 1 year">Diploma In Acupuncture Therapy - D.A.T.,1 year</option>
                        <option value="One Year,diploma inhijama & cupping with accupunture 2 years ">Diploma in hijama & cupping with accupunture 2 years </option>
