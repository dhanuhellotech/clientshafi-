...
const validationSchema = Yup.object().shape({
  name: Yup.string()
    ...
    .matches(/^\S+(\s+\S+){0,10}$/, 'Name must have at most 10 words')
  ...
  message: Yup.string()
  .trim()
  .matches(/^(?![\s\S]*?\b(?:https?|ftp|www)\S*\b)[\s\S]{0,200}$/, 'Message cannot contain links')
  .test(
    'word-count',
    'Message must have at most 200 words',
...
const containsLink = (text) => {
  ...
  const linkRegex = /(https?|ftp|www)\S*/gi;
              ...
              <div className="appointment-form rounded p-5">
                ...
                <Formik
                  ...
                  onSubmit={async (values, { setSubmitting, resetForm }) => {
                    try {
                      // Check if the message exceeds 200 words
                      const messageWords = values.message.trim().split(/\s+/);
                      if (messageWords.length > 200) {
                        // Truncate the message to 200 words
                        values.message = messageWords.slice(0, 200).join(' ');
                        alert("Message cannot exceed 200 words. It has been truncated.");
                      }
                      
                      // Submit the form with the possibly truncated message
        ...
        {errors.preferredAppointmentDateTime && touched.preferredAppointmentDateTime && (
          <div className="text-danger">{errors.preferredAppointmentDateTime}</div>
                    ...
                    <div className="col-xl-12">
                      <textarea
 className={`form-control py-3 border-primary bg-transparent text-black ${
  (errors.message && touched.message) || containsLink(values.message) ? 'is-invalid' : ''
}`}                        placeholder="Message"
                        name="message"
