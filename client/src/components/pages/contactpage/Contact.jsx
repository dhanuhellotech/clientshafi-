...
export default function Contact() {
  ...
  const handleChange = (e) => {
    ...
    // Truncate the name field to 20 characters
    const truncatedValue = name === 'name' ? value.slice(0, 20) : value;
    
    setFormData({ ...formData, [name]: truncatedValue });
    setFormErrors({ ...formErrors, [name]: '' }); // Reset error when user starts typing
    
    if (name === 'comments' && value.length > 200) {
      ...
      // Display an alert error message if the name exceeds 20 characters
